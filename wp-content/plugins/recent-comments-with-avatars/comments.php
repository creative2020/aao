...
function sa_comments_widget_init()
{
	...
	function sa_comments($args)
	{
		...
		echo $before_widget;
			...
			echo ($options['avatar'] == 'on' ? '<table' : '<ul') . ' id="recentcommentsavatar" class="recentcommentsavatar" cellspacing="0" cellpadding="0" border="0">';
			$avatar_size = empty($options['avatar-size']) ? 25 : $options['avatar-size'];
			if($comments) : foreach((array)$comments as $comment) :
				echo ($options['avatar'] == 'on' ? '<tr><td class="recentcommentsavatartop" style="height:"'.$avatar_size.'"px; width:"'.$avatar_size.'"px">' . get_avatar($comment, (empty($options['avatar-size']) ? 25 : $options['avatar-size'])) . '</td><td class="recentcommentstexttop">' : '<li class="recentcomments">');

				$url = get_comment_author_url();
				$author = get_comment_author();
				
				$author = short_author($author,' (q...', 4);
				...
				printf(' on %s', '<a href="'. get_comment_link($comment->comment_ID) . '">' . short_title($comment->comment_post_ID,'...', 5) . '</a>');
